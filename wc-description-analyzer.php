...
function wcda_admin_scripts($hook) {
    ...
    // Подключаем скрипт для размеров (только для функционала размеров)
    wp_enqueue_script('wcda-admin-script', plugin_dir_url(__FILE__) . 'admin/js/admin-script.js', array('jquery'), '2.1.0', true);
    
    wp_enqueue_style('wcda-admin-style', plugin_dir_url(__FILE__) . 'admin/css/admin-style.css', array(), '2.1.0');
}
